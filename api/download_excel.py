    ...
    try:
        ...
        if not all_data:
            return {
                'statusCode': 404,
                'headers': {'Content-Type': 'application/json'},
                'body': json.dumps({'error': '다운로드할 데이터가 없습니다.'}, ensure_ascii=False)
            }
        
        # 엑셀 파일 생성
        virtual_workbook, filename = create_excel_in_memory(all_data)
        excel_content = virtual_workbook.read()
        ...
        return {
            'statusCode': 500,
            'headers': {'Content-Type': 'application/json'},
            'body': json.dumps({'error': f'엑셀 생성 중 오류 발생: {str(e)}'}, ensure_ascii=False)
        }
